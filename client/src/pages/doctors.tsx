...
export default function Appointments() {
  ...
  const { data: appointments = [], isLoading } = useQuery<AppointmentWithDoctor[]>({
    queryKey: ["/api/appointments", doctorSearchData],
    enabled: !!doctorSearchData,
    queryFn: async () => {
      const response = await fetch(`/api/appointments?email=${encodeURIComponent(doctorSearchData)}`);
  ...
  const handleSearch = () => {
    if (doctorSearch.trim()) {
      setDoctorSearch(doctorSearch.trim());
    ...
    <div className="min-h-screen bg-clean-bg py-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-slate-gray">Find the best doctors for your needs</p>
        </div>
        ...
        {!doctorSearchData && (
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">Enter Doctor's Name or Specialty</h3>
              <p className="text-slate-gray">
                Please enter the doctor's name or specialty above to find a doctor.
              </p>
            </CardContent>
          </Card>
        )}
